...
export default function NextButton({ isToRight, onNext }: NextButtonProp) {
  const shadow = "var(--carousel-button-shadow)";
  return (
    <motion.button
      className="pointer-events-auto size-8 xs:size-10 md:size-11 rounded-full bg-slate-900/50 dark:bg-slate-50/50 flex justify-center items-center mx-2 md:mx-4"
      whileHover={{
        scale: 1.1,
        boxShadow: shadow,
      }}
