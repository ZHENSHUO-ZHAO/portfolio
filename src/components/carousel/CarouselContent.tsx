...
import type { CardData, LayoutData, OnNextCb } from "./carouselTypes";
...
export default function CarouselContent<T>({
  ...
  const [cardsData, setCardsData] = useState<CardData[]>(
    (Array.from({ length: cardCount }) as HTMLDivElement[]).map((_, i) => {
      let index =
        (i - initialMidIndex + layoutData.contentList.length) %
        ...
        Array.from({ length: offsetCount }).forEach(() => {
          if (offset < 0) {
            ...
            if (shiftCard && shiftCard.card) {
              ...
              const offset =
                (data.midIndex + initialMidIndex) * layoutData.snapSize;
  ...
  useEffect(() => {
    ...
    if (cardsHolderRef.current) {
      setCardsData(
        (Array.from(cardsHolderRef.current.children) as HTMLLIElement[]).map(
          (v, i) => {
            const data: CardData = { card: null, offset: 0, index: 0 };
            data.card = v;
    ...
    if (containerRef.current) {
      ...
      const onWheel = (e: WheelEvent) => {
        // When the scroll y dominant, let the scroll pass through so that the page can be scrolled when hovering the carousel.
          ...
          <LayoutContext value={layoutData as LayoutData<unknown>}>
            ...
            >
              {Array.from({ length: cardCount }).map((_, i) => (
                <Card key={i} dragX={dragX} data={cardsData[i]} />
              ))}
            </motion.ul>
          </LayoutContext>
        </motion.div>
