...
export default function Certification({ data }: { data: Certificate[] }) {
  return (
    <div className="grid min-[800px]:grid-cols-3 gap-4 min-[800px]:gap-8 lg:mx-8">
      {data.map((c, i) => (
        <CertificateItem data={c} index={i} key={`cert-${i}`} />
      ))}
    </div>
  );
}
...
function CertificateItem({
  ...
  const color: CardColor = colors[index];
  const animationProps = useResponsiveFadeIn(800, index);

  return (
    <motion.div {...animationProps}>
      <div className="size-full bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl md:rounded-3xl p-6 md:p-7 lg:p-8 hover:bg-white/15 active:bg-white/15 transition duration-300">
        <div className="flex flex-row min-[800px]:flex-col lg:flex-row items-start mb-4 min-[800px]:mb-7 lg:mb-6">
          <div
            style={
              {
                "--bg-image": `url(${data.iconUrl})`,
