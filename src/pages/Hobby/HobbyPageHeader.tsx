    ...
    <Header
      ...
      bgChildren={
        <HeaderBackground
          ...
          floatingLineGradient={["#7a2500", "#803a00", "#7a5a01"]}
        />
      }
      tagColor={{
    ...
    >
      ...
      <ol className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-10">
        {header.interest.map((i, j) => (
          <li
            key={`interest${j}`}
            className="flex gap-1 md:gap-2 items-center flex-wrap px-3 md:px-5 py-2 bg-white/50 dark:bg-slate-800/50 border-2 border-coffee-200 dark:border-amber-500/30 rounded-lg md:rounded-xl text-coffee-700 dark:text-amber-400 font-semibold text-sm md:text-base shadow-sm dark:shadow-coffee-500"
          >
            {i.icon && <i.icon />}
            <span>{i.text}</span>
          </li>
