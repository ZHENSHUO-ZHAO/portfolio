  ...
  return (
    <div className="relative space-y-8 md:space-y-12">
      {data.map((d, j) => {
        ...
        return (
          <motion.div {...getMultipleRowAnimation()} key={d.university}>
            <div
              className={`relative bg-linear-to-br ${color.bg} to-white dark:to-slate-900 border ${color.border} rounded-2xl md:rounded-3xl p-6 md:p-10 overflow-clip
              ${rightAnim_8px} hover:shadow-lg active:shadow-lg ${color.shadow} ${transition}`}
            >
              <div
                style={
                  {
                    "--bg-image": `url(${d.bgUrl})`,
                  } as React.CSSProperties
                }
                className="absolute inset-0 bg-(image:--bg-image) bg-cover bg-center mask-radial-[100%_100%] mask-radial-from-10% mask-radial-to-80% mask-radial-at-bottom-right opacity-30"
              />
                ...
                <div className="space-y-2">
                  {d.details.map((dt) => (
                    ...
                    >
                      ...
                      />
                      <p className="flex-1 text-secondary font-semibold text-sm md:text-base">
                        {dt.text}
                      </p>
                    </div>
                  ))}
